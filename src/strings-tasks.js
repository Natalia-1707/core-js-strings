...
function invertCase(str) {
  ...
  for (let i = 0; i < str2.length; i += 1) {
    const word = [];
    for (let j = 0; j < str2[i].length; j += 1) {
      if (typeof str2[i][j] !== 'number') {
        if (str2[i][j].toUpperCase() !== str2[i][j]) {
          word.push(str2[i][j].toUpperCase());
        } else {
          word.push(str2[i][j].toLowerCase());
        }
      }
    }
    arr.push(word.join(''));
...
function extractNameFromTemplate(value) {
  const value2 = value.replace(/[.,#!$%&;:{}=\-_`~()]/g, '');
...
function encodeToRot13(str) {
  ...
  const str2 = str.split('');
  let array = [];
  array = str2.map((element) => {
    const index = alphabet.indexOf(element);
    return index >= 0 ? rot13[index] : element;
  });
  return array.join('');
}
